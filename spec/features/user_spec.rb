...
describe 'User orders product' do
  ...
  it 'adds 2 products to cart and orders it' do
    ...
    click_on 'Zamawiam!'
    expect(page).to have_content('Złożono zamówienie. Szczegóły transakcji wysłano na adres e-mail.')
    visit orders_path
    expect(page).to have_content('Twoje zamówienia')
    expect(page).to have_selector(:link_or_button, 'Szczegóły')
  end
  
  it 'adds product by clicking on plus sign' do
    product = FactoryGirl.create(:product)
    visit root_url
    click_link('addtocart')
    expect(page).to have_content('Produkt dodany.')
    expect(page).to have_content('Twój koszyk')
    expect(page).not_to have_content('Nie ma jeszcze żadnych produktów.')
  end
